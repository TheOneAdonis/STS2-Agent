...
internal static class AiDefaultPrompts
{
    ...
    public static string ReadPrompt(IReadOnlyDictionary<string, string> promptMap, string? promptKey, bool isCombat)
    {
        if (TryReadPrompt(promptMap, promptKey, out var prompt))
        {
            return prompt;
        }

        var defaults = isCombat ? CachedPromptMaps.Value.Combat : CachedPromptMaps.Value.Route;
        return TryReadPrompt(defaults, promptKey, out prompt)
            ? prompt
            : string.Empty;
    }

    public static void EnsureLocalConfigExists(string configPath)
    {
        ...
        File.WriteAllText(configPath, JsonSerializer.Serialize(new AgentConfig(), JsonWriteOptions));
    }

    public static bool TryReadTemplateJson(out string json)
    {
        foreach (var path in GetTemplatePathCandidates())
        {
            if (!File.Exists(path))
            {
                continue;
    ...
    private static IEnumerable<string> GetTemplatePathCandidates()
    {
        ...
        {
            yield return Path.Combine(parentDir, "in-game-agent.json");
        }

        yield return Path.Combine(baseDir, "in-game-agent.json");
    }
